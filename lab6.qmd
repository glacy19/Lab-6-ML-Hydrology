...
# ensuring we replicate the results from the lm-base model
...
# Create a recipe to preprocess the data
rec2 <-  recipe(logQmean ~ soil_porosity + dom_land_cover, data = streamflow_train) %>%
  # Log transform the predictor variables (aridity and p_mean)
  step_log(all_predictors()) %>%
  # Add an interaction term between aridity and p_mean
  step_interact(terms = ~ soil_porosity:dom_land_cover) |> 
  # Drop any rows with missing values in the pred
  step_naomit(all_predictors(), all_outcomes())
...
# Prepare the data
baked_stream_data <- prep(rec, streamflow_train) |> 
  bake(new_data = NULL)
```
...
# Interaction with lm
#  Base lm sets interaction terms with the * symbol
lm_stream_base <- lm(logQmean ~ soil_porosity * dom_land_cover, data = baked_stream_data)
summary(lm_stream_base)


# Sanity Interaction term from recipe ... these should be equal!!
summary(lm(logQmean ~ soil_porosity + dome-land_cover + soil_x_land, data = baked_stream_data))
